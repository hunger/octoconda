...
| `tag-prefix`     | no       | Custom prefix (or list of prefixes) to strip from release tags before version parsing. Accepts a single string (`"jdk-"`) or an array (`["jdk-", "graal-"]`). When set, only tags starting with one of the prefixes are considered; the first matching prefix is removed to extract the version, and tags matching none are skipped. |
